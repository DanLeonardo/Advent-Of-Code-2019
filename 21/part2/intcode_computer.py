...
class IntcodeComputer:
    ...
    def load_program(self, file_name):
        with open(file_name) as file:
            ...
            self.base_program = [op for op in self.program]

    def reset(self):
        if self.echo:
            print('Intcode: Reseting computer')
        self.program = [op for op in self.base_program]
        self.position = 0
        self.rel_base = 0
        self.input_list =[]
        self.output_values = []
    ...
    def run(self):
        ...
        if self.finished:
            print('IntcodeComputer is finished. Reset before running again.')
    ...
    def read_ascii(self, input):
        self.input_list = [ord(char) for char in input]
        self.run()
